 ...
 * Vendored into einf from IST-DasLab/marlin@master (Apache-2.0,
 * Elias Frantar): marlin/marlin_cuda_kernel.cu. einf additions are marked
 * with "einf:" below -- three extra CALL_IF instantiations for
 * (thread_n=128, thread_k=128) with thread_m_blocks 2-4, required because
 * Qwen head/intermediate dimensions (N=1152, N=896) are not multiples of
 * 256, so the stock prefill configuration (64, 256) cannot be dispatched.
 *
