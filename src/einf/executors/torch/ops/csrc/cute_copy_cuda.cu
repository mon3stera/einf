...
__global__ void cute_copy_kernel(
    SourceTensor source,
  ...
  auto frag = make_tensor_like(partition_src);

  copy(partition_src, frag);
  copy(frag, partition_dst);
}
...
at::Tensor cute_copy_cuda(const at::Tensor& input) {
  ...
  const auto block_shape = make_shape(Int<kBlockRows>{}, Int<kBlockCols>{});
  ...
  cute_copy_kernel<<<blocks, threads, 0, stream>>>(
      tiled_source,
      tiled_destination,
      thread_layout);
